...
class PengabdianController extends Controller
{
    ...
    public function updatePengabdianView($id){
        // Log::info($id);
        $itemInfo = Pengabdian::where('id',$id)->first();
        $people = User::where('roles','!=',1)->where('id','!=',$itemInfo->user_id)->orderBy('username','ASC')->get();

      
        return Inertia::render('Pengabdian/update-visitor', [
    ...
    public function updatePengabdian(Request $request){
        ...
        try{
            ...
            $itemInfo = Pengabdian::with(['user'])->where('id', $itemId)->where('deleted_at', null)->first();
            ...
            if($itemInfo->permission === 1){
                ...
                $destinationDir = storage_path('app/private/uploads/'. Auth::user()->fakultas . '/' . ($prodi !=null ? $prodi . '/' : '') .$itemInfo->tahun_data.'/'.$itemInfo->user->username.'/pengabdian');
                // Create the directory if it doesn't exist
                    if (!file_exists($destinationDir)) {
                        mkdir($destinationDir, 0755, true); // Recursive directory creation with permissions
                    }
                $destinationPath = storage_path('app/private/uploads/'. Auth::user()->fakultas . '/' . ($prodi !=null ? $prodi . '/' : '').$itemInfo->tahun_data.'/'.$itemInfo->user->username .'/pengabdian/'. $newFilename);
                ...
                if (Storage::disk('public')->exists($relativeFilePath)&& file_exists($old_file)) {
                    if (copy($old_file, $destinationPath)) {
                        ...
                        $updateData['link_pengabdian'] = 'uploads/'. Auth::user()->fakultas . '/' . ($prodi !=null ? $prodi . '/' : '') .$itemInfo->tahun_data.'/'.$itemInfo->user->username.'/pengabdian/'.$newFilename; 
                ...
                $destinationDir = public_path('storage/uploads/'. Auth::user()->fakultas . '/' . ($prodi !=null ? $prodi . '/' : '') .$itemInfo->tahun_data.'/'.$itemInfo->user->username.'/pengabdian');
                ...
                $updateData['link_pengabdian'] = '/storage/uploads/'. Auth::user()->fakultas . '/' . ($prodi !=null ? $prodi . '/' : '') .$itemInfo->tahun_data.'/'.$itemInfo->user->username.'/pengabdian/'.$newFilename;
    ...
    public function updatePengabdianVisitor(Request $request){
        $itemId     = $request->input('item_id');
        $auth = Auth::user()->id;
        ...
        try{
            ...
            $this->code = 0;
            $this->message = "Change visitor data Pengabdian Success";
        }catch(Exception $e){
            $this->message = $e->getMessage();
        }
