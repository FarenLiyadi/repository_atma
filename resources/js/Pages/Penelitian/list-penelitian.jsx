...
export default function listpenelitian({ auth }) {
    ...
    const [guest_mode, setguest_mode] = useState(0);
    ...
    async function deletePengabdian(e, id) {
        ...
        if (confirm("Hapus Penelitian?")) {
            ...
            try {
                const response = await axios.post("/delete-penelitian", data);
    ...
    async function updatePengabdian(e, id) {
        e.preventDefault();
        if (confirm("update permission Penelitian?")) {
            const data = {
                actor_id: auth.user.id,
                item_id: id,
            };
            ...
            try {
                const response = await axios.post("/update-penelitian", data);
                if (response.data.code !== 0) {
                    toast.error(response.data.msg, {
                        position: "top-right",
                        autoClose: 3000,
    ...
    function handleOpen(user) {
        setOpen(true);
        // console.log(JSON.parse(visit));

        setVisitor(JSON.parse(user.visitor));
        setJudul(user.judul_data);
        setguest_mode(user.guest_mode);
        setpercobaan(user.percobaan);
        setkode_sandi(user.kode_sandi);
        setId(user.id);
    }
    ...
    async function getListUser(page = 1, length = 10, username, role, tahun) {
        ...
        try {
            const response = await axios.get("/list-penelitian-request", {
                params: parameter,
            });
            if (response.data.code !== 0) {
                toast.error(response.data.msg, {
                ...
                <form>
                    <Card className="mx-auto w-full">
                        <CardBody className="flex flex-col gap-4">
                            ...
                            >
                                Mode sharing tamu :{" "}
                                {guest_mode == 0 ? "tidak aktif" : "aktif"}
                            </Typography>
                            {}
                            {guest_mode ? (
                                <p
                                    onClick={() =>
                                        handleCopyClick(
                                            `127.0.0.1:8000/guest/${id}`
                                        )
                                    }
                                    className="bg-blue-400 text-center rounded-lg text-white w-24 px-2 py-1.5 font-normal"
                                >
                                    copy link
                                </p>
                            ) : (
                            ...
                            <Typography
                                ...
                                color="gray"
                            >
                                kode sandi : {kode_sandi ?? "-"}
                            </Typography>
                            <table>
                                <thead className="border">
                                    <tr>
                                        <th className="border max-w-2">No</th>
                                ...
                                <tbody className="border">
                                    {visitor
                                        ? visitor.map((e, index) => {
                                              return (
                                                  <tr>
                                                      <td className="text-center max-w-2 border">
                                                          {index + 1}
                                                      </td>
                                                                        ...
                                                                        <div className="relative z-50">
                                                                            ...
                                                                            {user.user_id ==
                                                                                auth
                                                                                    ...
                                                                                    .id && (
                                                                                <a
                                                                                    href={`update-penelitian/${user.id}`}
